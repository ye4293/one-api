...
func SetRelayRouter(router *gin.Engine) {
	...
	// 用户请求级 LLM 指标。挂在根引擎上（与 CORS 同层），但中间件内部只对真正走了 relay
	// 的请求计数 —— 判据是 gin context 里有没有 model/original_model 键，
	// 而不是路径白名单（relay 路由有 19 个前缀，白名单会随新增路由失效）。
	router.Use(middleware.RelayMetrics())
	...
	relayV1Router.Use(middleware.RelayPanicRecover(), middleware.TokenAuth(), middleware.Distribute())
	{
		...
		relayV1Router.POST("/flux-tools/video-upscale-v1", controller.RelayVideoGenerate)
