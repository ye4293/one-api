...
// GPT-5.4/5.5/5.6 系列的价格倍率与 long-context / cache-write 计费校验。
// 定价来源：https://openai.com/api/pricing 及 prompt caching 文档。
// 换算规则：ModelRatio = 输入价格($/1M) / 2；CompletionRatio = 输出价/输入价；
//
//	CacheRatio(读) = 缓存读取价/输入价；CacheWriteRatio(写) = 缓存写入价/输入价或普通输入价倍率。
func TestGPT5FamilyRatios(t *testing.T) {
	cases := []struct {
		model      string
		modelRatio float64
		completion float64
		...
		{"gpt-5.4", 1.25, 6, 0.1, 1.0},
		{"gpt-5.4-2026-03-05", 1.25, 6, 0.1, 1.0},
		{"gpt-5.4-pro", 15, 6, 1.0, 1.0},
		{"gpt-5.4-pro-2026-03-05", 15, 6, 1.0, 1.0},
		{"gpt-5.5", 2.5, 6, 0.1, 1.0},
		{"gpt-5.5-pro", 15, 6, 1.0, 1.0},
...
// long-context 倍率：输入×2、输出×1.5（GPT-5.4/5.5/5.6 family）；未注册模型恒为 1x。
func TestGPT5FamilyLongContextMultipliers(t *testing.T) {
	models := []string{"gpt-5.4", "gpt-5.4-pro", "gpt-5.5", "gpt-5.5-pro", "gpt-5.6-sol"}
	for _, model := range models {
		// short 档
		mults := GetLongContextMultipliers(model, 272000)
		if mults.InputMultiplier != 1.0 || mults.OutputMultiplier != 1.0 {
			t.Errorf("%s 边界 272000（short）应为 (1.0, 1.0)，got (%.1f, %.1f)", model, mults.InputMultiplier, mults.OutputMultiplier)
		}
		// long 档
		mults = GetLongContextMultipliers(model, 272001)
		if mults.InputMultiplier != 2.0 || mults.OutputMultiplier != 1.5 {
			t.Errorf("%s long 档应为 (2.0, 1.5)，got (%.1f, %.1f)", model, mults.InputMultiplier, mults.OutputMultiplier)
		}
	}
	// 未注册模型
...
func TestGPT5FamilyBillingSamples(t *testing.T) {
	// responses 样本：gpt-5.5 非 pro，cache_write 无额外加价，等同普通输入
	{
		const (
			modelRatio      = 2.5
			completionRatio = 6.0
		...
		if quota != 211 {
			t.Errorf("gpt-5.5 responses 样本 quota = %d, want 211", quota)
		}
	}
	...
	// responses 样本：input=11(含 cached=5, write=3), output=13，sol 模型
	{
		...
		quota := int64(sum / 1000000 * 2 * 1 * 500000)
		if quota != 213 {
			t.Errorf("gpt-5.6-sol responses 样本 quota = %d, want 213 (=$0.000426)", quota)
		}
	}
}
