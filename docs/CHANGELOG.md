...
- **处置：两者在两个场景下都「不动」，但理由不同**。pendingRemove 不删是因为模型可用（429）或只是暂时不可用（503），删了是误伤；pendingAdd 不加是因为 —— **很多网关的限流检查发生在模型路由之前，对不存在的模型也会返回 429**，直接当 alive 会把不存在的模型加进列表，不加的代价只是延迟一轮。
- **为什么不并入 inconclusive**: 处置虽同为「不动」，但运维含义完全不同。上线看日志时「限流 45 个 + 无结论 5 个」和「无结论 50 个」是两回事，前者说明该渠道正被限流、探测结果基本有效，后者说明探针真的没拿到信息。轮末统计日志已加上这两个计数。
- **中止逻辑重写**: `note429` → `noteResult`，只对**确定性的渠道级故障**中止，且第一次就停 —— 401（key 无效）/ 403（无权限）/ 402（欠费），这几种重试不可能成功，继续探只是白耗全局预算。429 / 503 / 5xx / 超时**一律不中止**，由次数（30/渠道）与时长（120s/渠道）预算兜底。中止时记录 `abortReason` 供日志排查。
- **顺带修掉原实现的两个缺陷**: (1) 超时路径 `StatusCode=0` 会让 `note429(false)` **重置连续计数**，而超时恰恰是限流最常见的表现形式，导致 `429 → 超时 → 429` 永远触发不了中止；(2)「连续」定义过严，`429 → 成功 → 429` 这种典型的限流边缘状态永远累计不到阈值。新逻辑不依赖连续计数，两个缺陷一并消失。
...
- **验证**: `controller` + `model` 两包 **216 个用例全绿、0 失败**。新增 429/503 各 2 组判定用例、渠道中止的三类场景（401/403/402 立即中止、429/503 五次不中止、5xx/超时不中止）、中止原因不被覆盖。**实测有效性**：临时去掉 429 判定分支 → 相关用例立刻 FAIL。`go build ./...`、`go vet ./...`（退出码 0）通过。
