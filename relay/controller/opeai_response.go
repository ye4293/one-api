...
func RelayOpenaiResponseNative(c *gin.Context) *model.ErrorWithStatusCode {
	...
	if responseID := c.GetHeader("X-Response-ID"); responseID != "" && openaiResponseRequest.Id == "" {
		openaiResponseRequest.Id = responseID
		// 重新序列化请求体
		if modifiedBody, marshalErr := json.Marshal(openaiResponseRequest); marshalErr == nil {
			originRequestBody = modifiedBody
			logger.SysLog(fmt.Sprintf("[OpenAI Response] Set id from X-Response-ID header: %s", responseID))
		} else {
			logger.SysError(fmt.Sprintf("[OpenAI Response] Failed to marshal modified request: %v", marshalErr))
		}
	}
