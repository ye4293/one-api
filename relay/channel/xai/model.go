...
// GrokVideoError 视频 API 错误信息
type GrokVideoError struct {
	Code    string `json:"code,omitempty"`
...
type GrokVideoResult struct {
	Status   string           `json:"status,omitempty"`   // 状态：pending、done、failed
	...
	Error    *GrokVideoError  `json:"error,omitempty"`    // 错误信息（失败时）
}
